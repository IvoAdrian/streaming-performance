...
# Be in the directory: ~/github/power-predict-reactor

export projectDir=~/github/power-predict-reactor
...
case "$PLATFORM" in 
	...
	native) target/graalvm-native-image/power-predict-reactor $LOAD > Benchmark_Results/$PLATFORM$LOAD-latencies.txt &
	export RUN_PID=$!
